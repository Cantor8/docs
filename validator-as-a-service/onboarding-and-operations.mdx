...
Partners can always choose to run their own Canton validator node in their own infrastructure. That path gives maximum internal control, but it also requires the partner to own DevOps, monitoring, release updates, traffic management, and operational support.
...
<Steps>
  <Step title="Confirm validator scope">
    C8 and the partner confirm the target network, expected usage, operating requirements, and whether the validator supports a wallet, token, settlement, or custom workflow.
  </Step>
...
- **Release management** — applying required updates and keeping the validator aligned with network expectations.
...
<Check>
  The node should be deployed in the isolated environment, registered for the target network, allowlisted where required, monitored, and connected to the partner workflow that depends on it.
</Check>
...
<CardGroup cols={2}>
  <Card title="Discuss validator scope" icon="compass" href="mailto:integrations@cantor8.io">
    Share the target network, product workflow, and expected launch timeline.
  </Card>
